...
NODE1_IP="${NODE1_IP:-${NODE1##*@}}"
NODE2_IP="${NODE2_IP:-${NODE2##*@}}"
PORT="${PORT:-8004}"
MP="${MP:-29577}"
K="${K:-16}"
BASE="${BASE:-/home/admin/qwen-repro}"
MODEL_DIR="${MODEL_DIR:-$BASE/unsloth-nvfp4-bf16head}"
DRAFT_DIR="${DRAFT_DIR:-${DRAFT_MODEL_DIR:-$BASE/dflash2}}"
IMAGE="${IMAGE:-local/vllm-dflash2-pr52816:v4}"
CACHE_DIR="${CACHE_DIR:-$BASE/vllm-cache}"
...
sed "s/num_speculative_tokens\":[0-9]*/num_speculative_tokens\":$K/" \
  "$SCRIPT_DIR/qwen-dflash2-rank.sh" > /tmp/qwen-dflash2-rank.sh
ssh -o ConnectTimeout=8 "$NODE1" "mkdir -p $BASE"
ssh -o ConnectTimeout=8 "$NODE2" "mkdir -p $BASE"
...
ssh -o ConnectTimeout=8 "$NODE2" "RANK=1 HOST_IP=$NODE2_IP MASTER_ADDR=$NODE1_IP MASTER_PORT=$MP \
  MODEL_DIR=$MODEL_DIR DRAFT_DIR=$DRAFT_DIR PORT=$PORT IMAGE=$IMAGE CACHE_DIR=$CACHE_DIR bash $BASE/qwen-dflash2-rank.sh"
ssh -o ConnectTimeout=8 "$NODE1" "RANK=0 HOST_IP=$NODE1_IP MASTER_ADDR=$NODE1_IP MASTER_PORT=$MP \
  MODEL_DIR=$MODEL_DIR DRAFT_DIR=$DRAFT_DIR PORT=$PORT IMAGE=$IMAGE CACHE_DIR=$CACHE_DIR bash $BASE/qwen-dflash2-rank.sh"
...
for i in $(seq 1 90); do
  sleep 5
  HEALTH_IP="${NODE1##*@}"
  c=$(curl -s -o /dev/null -w '%{http_code}' --max-time 4 "http://$HEALTH_IP:$PORT/health" || true)
  [ "$c" = "200" ] && break
done
